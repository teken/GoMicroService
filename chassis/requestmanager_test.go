...
func TestNewRequest(t *testing.T) {
	...
	manager := NewRequestManager(nil, nil, nil)
	...
	assert.NoError(t, manager.RegisterRequestHandler("/products/{key}", "get", func(requestContext RequestContext) RequestResponse {
	...
	assert.NoError(t, manager.RegisterRequestHandler("/type/{key}", "get", func(requestContext RequestContext) RequestResponse {
	...
	assert.NoError(t, manager.RegisterRequestHandler("/articles/{category}", "get", func(requestContext RequestContext) RequestResponse {
	...
	assert.NoError(t, manager.RegisterRequestHandler("/articles/{category}/{id:[0-9]+}", "get", func(requestContext RequestContext) RequestResponse {
	...
	assert.NoError(t, manager.RegisterRequestHandler("/panic", "get", func(requestContext RequestContext) RequestResponse {
		panic("test")
		return NotImplementedResponse
	}))

	manager.RegisterUnhandledRequestHandler(func(requestContext RequestContext) RequestResponse {
